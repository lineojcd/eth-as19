...
stripchart(weight ~ sire, vertical = TRUE, pch = 1, data = animals)
...
coef(fit.lme)
ranef(fit.lme) ## "estimated" random effects
...
## Exact tests for variance component
library(RLRsim)
class(fit.lme) <- "lmerMod" ## hack to make it working
exactRLRT(fit.lme) ## currently not working because of bug in RLRsim
ranova(fit.lme)    ## conservative test for variance components
